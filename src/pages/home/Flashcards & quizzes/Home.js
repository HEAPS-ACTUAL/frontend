...
import {
  ...
  trackWrongFileType,
} from "../../../services/PostHogAnalyticsServices";

import SelectPagesModal from "../../../components/ui/SelectPagesPopUp";
...
function Home() {
  ...
  async function filterFlashcards() {
    const filteredFlashcardList = allFlashcards.filter((flashcard) =>
      flashcard["TestName"].toLowerCase().includes(search.toLowerCase())
    );
  ...
  function handleFileUpload(event) {
    ...
    if (isVerified) {
      if (testName.trim() === "") {
        ...
        testTypeChecked["Flashcard"] === false &&
        testTypeChecked["Quiz"] === false
      ) {
  ...
  return (
    <div className={styles.container}>
      ...
      <div className={styles.yourFlashcards}>
        ...
        <div className={styles.flashcardSearchBar}>
          {" "}
          <SearchBar
            setSearch={setSearch}
        ...
        <div className={styles.flashcardList}>
          {allFlashcards.length === 0 ? (
            <p className={styles.noQuizMessage}>
              {" "}
              You do not have any flashcards. Create a flashcard above!{" "}
            </p>
          ) : filteredFlashcards.length === 0 ? (
            <p className={styles.noQuizMessage}>
              {" "}
              Sorry, we couldn't find any flashcard that matches your search{" "}
            </p>
          ) : (
